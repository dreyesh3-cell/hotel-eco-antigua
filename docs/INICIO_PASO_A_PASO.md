...
6. **Publica la aplicación en una cuenta de nube del equipo.** La dirección local `localhost` solo funciona en tu computadora; no sirve como enlace público. La aplicación necesita tanto la página como la API y su base de datos. Sigue la sección “Despliegue automatizado” en [`FASE2_AUTOMATIZACION_Y_PLATAFORMA.md`](FASE2_AUTOMATIZACION_Y_PLATAFORMA.md), usando una cuenta administrada por el equipo. Abre el enlace publicado en una ventana privada para comprobar que funciona.
